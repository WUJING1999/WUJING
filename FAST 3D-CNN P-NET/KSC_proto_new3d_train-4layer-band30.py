...
from KSC_data_loaderBS30 import KSC
...
def test(support, query, proto_network):
    ...
    sample_features = sample_features.view(n_test_way, n_test_shot, 64*49*3)
...
def main():
    ...
    save_path = './model/KSC/KC_proto_3d-5shotband30layer3-64output.pth'.format(n_way, n_shot)
    proto_network.train()
    ...
    for ep in range(n_epochs):
        ...
        for epi in range(n_episodes):
            ...
            sample_features = sample_features.view(n_way, n_shot, 64*49*3)
            sample_features = torch.mean(sample_features, 1).squeeze(1)
