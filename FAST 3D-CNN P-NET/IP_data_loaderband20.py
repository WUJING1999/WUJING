...
class IndianPines():
    site3_filename = 'data/Indian_pines_corrected.mat'
    site3_gr_filename = 'data/Indian_pines_gt.mat'
    site3_gr_classes_filename = './palette/IP_palette.csv'

    def __init__(self, width):
        self.raster_data = sio.loadmat(IndianPines.site3_filename)['indian_pines_corrected']
        self.image = self.raster_data
        self.windowSize = width
        self.margin = int((width - 1) / 2)
        self.ground_truth = sio.loadmat(IndianPines.site3_gr_filename)['indian_pines_gt']
    ...
    def get_patch(self, index, p=11):
        ...
        selected = [121, 152, 43, 36, 25, 99, 48, 98, 179, 112, 74, 91, 171, 65, 26, 132, 197, 100, 76, 34, 68, 161, 40, 60, 198, 50,
                    66, 85, 154, 8, 101, 147, 12, 186, 118, 71, 93, 89, 18, 35, 128, 96, 78, 55, 7, 173, 59, 62, 11, 104, ]
    ...
    def getPatches(self, indexes):
        patchData = np.zeros([len(indexes), self.windowSize, self.windowSize, 50], dtype=np.float32)
        selected = [121, 152, 43, 36, 25, 99, 48, 98, 179, 112, 74, 91, 171, 65, 26, 132, 197, 100, 76, 34, 68, 161,
                    40, 60, 198, 50, 66, 85, 154, 8, 101, 147, 12, 186, 118, 71, 93, 89, 18, 35, 128, 96, 78, 55, 7, 173, 59, 62, 11, 104, ]
