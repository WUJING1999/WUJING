...
def test(support, query, proto_network):
    ...
    sample_features = sample_features.view(n_test_way, n_test_shot, 64*63)
...
def main():
    ...
    Xval = np.load('./data/IPXtestWindowSize11testRatio0.95.npy')
    Xtrain = np.load('./data/IPXtrainWindowSize11testRatio0.95.npy')
    yval = np.load('./data/IPytestWindowSize11testRatio0.95.npy')
    ...
    for ep in range(n_epochs):
        ...
        for epi in range(n_episodes):
            ...
            sample_features = sample_features.view(n_way, n_shot, 64*63)
            sample_features = torch.mean(sample_features, 1).squeeze(1)
