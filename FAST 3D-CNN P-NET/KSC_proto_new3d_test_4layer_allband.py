...
from KSC_data_loader import KSC
...
class ProtoNetwork(nn.Module):
    ...
    def __init__(self, output_dim):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=0),
            nn.ReLU(),
            nn.BatchNorm3d(num_features=8),
            #nn.MaxPool3d((2, 1, 1))
        )
        self.layer2 = nn.Sequential(
            nn.Conv3d(in_channels=8, out_channels=16, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=0),
            nn.ReLU(),
            nn.BatchNorm3d(num_features=16),
            #nn.MaxPool3d((2, 1, 1))
        )
        self.layer3 = nn.Sequential(
            nn.Conv3d(in_channels=16, out_channels=32, kernel_size=(7, 3, 3), stride=(2, 1, 1), padding=0),
            nn.ReLU(),
...
def test(support, query, proto_network):
    ...
    sample_features = sample_features.view(n_test_way, n_test_shot, 64*54)
...
def main():
    seed_everything()
    width, height, channel = 11, 11, 176
    zgff = KSC(width)
    name = "KC-allband"
    Xtest = np.load('./data/KCXtestWindowSize11testRatio0.95.npy')
    Xtrain = np.load('./data/KCXtrainWindowSize11testRatio0.95.npy')
    ytest = np.load('./data/KCytestWindowSize11testRatio0.95.npy')
    ...
    save_path = './model/KSC/KC_proto_3d-5shotallband-64output.pth'.format(n_way, n_shot)
    ...
    target_name = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J ', 'K', 'L', 'M', ]
    ...
    plt.savefig('./map/{}_confusion_matrixALLband.png'.format(name))
