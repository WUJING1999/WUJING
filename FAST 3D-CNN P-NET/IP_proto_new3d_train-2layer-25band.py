...
def test(support, query, proto_network):
    ...
    sample_features = sample_features.view(n_test_way, n_test_shot, 64*49*6)
...
def main():
    ...
    save_path = './model/IP/IP_proto_3d-5shotband40-64output.pth'.format(n_way, n_shot)
    proto_network.train()
    ...
    for ep in range(n_epochs):
        ...
        for epi in range(n_episodes):
            ...
            sample_features = sample_features.view(n_way, n_shot, 64*49*6)
            sample_features = torch.mean(sample_features, 1).squeeze(1)
